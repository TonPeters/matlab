...
for ii=1:length(varargin)
    ...
    if issystem(inputii)
        ...
        systems{nsys}.markersize=6;
        systems{nsys}.marker='none';
    end
    %detect formatting
    if ischar(inputii)
        ...
        if(~isempty(temp.marker)),     systems{nsys}.marker     = temp.marker;     end
        if(~isempty(temp.markersize)), systems{nsys}.markersize = temp.markersize; end
    end
    %detect options
    if iscell(inputii)
        options=inputii;
...
try wrapping = options{7};   catch, wrapping='on';      end;
...
for ii=1:nsys
    ...
    for nn=1:sys.noutputs
        for mm=1:sys.ninputs            
            %magnitude plots
            if(plotmag)
                semilogx(handles((plotmag+plotphase)*nn-plotphase,mm),sys.f,db(squeeze(sys.mag(nn,mm,:))),sys.linetype,'color',sys.color,'linewidth',sys.linewidth,'marker',sys.marker,'markersize',sys.markersize);                                           
            end
            %phase plots
            if(plotphase)                
                semilogx(handles((plotmag+plotphase)*nn,mm),sys.f,squeeze(sys.phase(nn,mm,:)),sys.linetype,'color',sys.color,'linewidth',sys.linewidth,'marker',sys.marker,'markersize',sys.markersize);                                 
            end
        end
    end
end
...
if(magmin~=magmax)
    ws=(magmax-magmin)*0.1; %fancy whitespace
    set(ha,'ylim',[magmin-ws magmax+ws]);
end
%phase axis
ha=phasehandles(:);
for ii=1:length(ha)
...
if(phasemin~=phasemax)
    ws=(phasemax-phasemin)*0.1; %fancy whitespace
    set(ha,'ylim',[phasemin-ws phasemax+ws]);
end
...
nticks=4; 
dtick=90;%initial dtick
while floor((phasemax-phasemin)/dtick)<(nticks-1),dtick=0.5*dtick;end
while floor((phasemax-phasemin)/dtick)>(nticks-1),dtick=2*dtick;end
% set(phasehandles,'ytick',-36000:dtick:36000);  
...
set(gcf,'color',[0.8 0.8 0.8]); %fancy blue
...
% Custom colors definitions
    custom= {...
        ...
        'bluea',     [99 184 255]./255;
        'bluec',     [16 78 139]./255;
        'blueb',     [30 144 255]./255;
        'greena',     [0 255 0]./255;
        'greenb',     [0 139 0]./255;
        'greenc',     [0 100 0]./255; 
        'redb',     [255 0 0]./255;
