...
%     figure(fig2);
%     plot(q0_meas./pi*180,tau_meas,':','color',ps.list_div_XL{i,4}); hold all; 
...
%         indi = find(q0_meas(indices)<57/180*pi);
        ...
        % filter data
        tau_m_filt = filter_data(tau_m,2,0.05);
        
        % filter data
%         n_tmp = 3000;
%         tau_mean_s = mode(tau_m(1:1000))
%         tau_mean_e = mode(tau_m(end-1000:end))
%         tau_m_tmp = [ones(n_tmp,1).*tau_mean_s;tau_m;ones(n_tmp,1).*tau_mean_e];
%         tau_m_filt_tmp = lowpass(tau_m_tmp,1/0.001,0.1);
%         tau_m_filt = tau_m_filt_tmp(n_tmp+1:end-n_tmp);
        ...
        figure(fig2);
        plot(q0_meas(indices)./pi*180,tau_meas(indices),':','color',ps.list_div_XL{i,4}); hold all; 
        % plot data, filtered and sampled
        figure(fig2);
        plot(q0_m./pi*180,tau_m_filt,'-','color',ps.list_div_XL{i,6}); hold all;
        plot(q0_m_s_i./pi*180,tau_m_s{k},'--','color',ps.list_div_XL{i,6},'linewidth',ps.linewidth); hold all;
...
ylim([0 0.4]);
%% save data
% save('../Torso_model/Legs_2/Final/data_legs_new','sdata');
