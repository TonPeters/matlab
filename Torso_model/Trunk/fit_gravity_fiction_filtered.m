...
% clear parameters
clearvars -except l1 lcm1 lF th_r m1 m2 th_2_min th_2_max g m1_lcm1 th_r2
plotsettings
%% load data to fit
PC = 'ton/git_ton';
...
%     end
    ...
    q_m_s = linspace(q_min+0.01,q_max-0.01,n_s).';
    ...
    tau_p = interp1(q_m_f,tau_m_filt_p(IA),q_m_s);
    [q_m_f,IA,IC] = unique(q_m(indi_n));
    tau_n = interp1(q_m_f,tau_m_filt_n(IA),q_m_s);
    ...
    plot((q_m_s-data_offset)./pi*180,tau_grav{i},'color',ps.list{i}); hold all;
    plot((q_m_s-data_offset)./pi*180,tau_fric_p{i},'--','color',ps.list{i}); hold all;
    plot((q_m_s-data_offset)./pi*180,tau_fric_n{i},'-.','color',ps.list{i}); hold all;
    ...
    problem = createOptimProblem('lsqnonlin','objective',...
        @(param)costfunc_gravity_nonlin_V1(param,q_m_s,tau_grav{i},m2,data_offset),...
    ...
    % Moment needed
    M_joint = (g.*(m1_lcm1+m2*l1).*cos(q2-data_offset+2/180*pi));
    ...
    figure; scr rt;
    % subplot(2,1,1)
    plot((q_meas-data_offset)./pi*180,tau_meas,':','color',ps.list{i}); hold all;
    plot((q_m_s-data_offset)./pi*180,tau_grav{i},'--','color',ps.list{i}); hold all;
    plot((q2-data_offset)./pi*180,V_mod,'color',ps.list{i});
