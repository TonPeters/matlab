...
range = max_spindle1-min_spindle1;

%% calculate the desired gear ratio
% gear by force
F_bend = 1700;      % force to cover bending region
F_complete = 2500;  % force to cover complete range
...
fprintf(format_spec1,F_bend,gear_des_bend,gear_nom_speed,nom_speed)
fprintf(format_spec1,F_complete,gear_des_complete,gear_nom_speed,nom_speed)
fprintf('\n');

%% calculate the desired motor torque
tau_bend = F_bend/r_sp/r_gear1;
tau_complete = F_complete/r_sp/r_gear1;
...
% gear 2.5:1 (currently used)
gear =                  r_gear1;
duration =              4;
force = tau_nom_max*gear*r_sp;
omega_m = range/(duration-1)*r_sp*gear;
rpm = omega_m/2/pi*60;
resolution = 2*pi/enc_counts/r_gear1/r_sp*1000;
...
yl2 = get(ax(2),'ylim');
axes(ax(1));
plot(r_gear1,yl1(1),'*','color',ps.list_div_XL{4,8});
axes(ax(2));
plot(r_gear1,yl2(1),'*','color',ps.list_div_XL{4,8});

save_fig = false;
if save_fig
    scale =0.8;
    dir = '/home/ton/Dropbox/Linux/Report_final/Images/Torso/';
    save_report(fig1,dir,'leg_gear','customleg',[6 6].*scale);
end
