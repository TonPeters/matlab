...
% Joint Torque legs
T_leg = g.*(cos(q0).*P1+...             
    (dq1_q0-1).*cos(q1-q0).*P2+...     
    (1-dq1_q0).*cos(q2-q1+q0).*P3);
...
figure; 
subplot(2,2,1)
plot(squeeze(q0(:,:,1)).'./pi*180,squeeze(V_1(:,:,1))); hold all; 
ylabel('Voltage (+ vel)'); ax1 = gca;
subplot(2,2,3)
plot(squeeze(q0(:,:,2)).'./pi*180,squeeze(V_1(:,:,2)));
ylabel('Voltage (- vel)');ax3 = gca;
xlabel('angle joint 1 [deg]');
subplot(2,2,2)
plot(squeeze(q2(:,:,1))./pi*180,squeeze(V_2(:,:,1)));  ax2 = gca;
hold all;
subplot(2,2,4)
plot(squeeze(q2(:,:,1))./pi*180,squeeze(V_2(:,:,2))); ax4 = gca;
xlabel('angle joint 2 [deg]');
linkaxes([ax1 ax3],'xy')
linkaxes([ax2 ax4],'xy')

% define colormap
cmax1 = max(max(max(max(abs(V_1)))));
cmin1 = min(min(min(min(abs(V_1)))));
cmax2 = max(max(max(max(abs(V_2)))));
cmin2 = min(min(min(min(abs(V_2)))));
cm_length = 60;
CV_1 = fix((abs(V_1)-cmin1)./(cmax1-cmin1).*cm_length)+1;
CV_2 = fix((abs(V_2)-cmin2)./(cmax2-cmin2).*cm_length)+1;

figure; scr l;
subplot(2,1,1)
surf(squeeze(q0(:,:,1))./pi*180,squeeze(q2(:,:,1))./pi*180,squeeze(V_1(:,:,1)),squeeze(CV_1(:,:,1)),...
    'CDataMapping','direct');
title('positive velocity');
xlabel('q_0 [deg]'); ylabel('q_2 [deg]'); zlabel('input V1 (+vel)');
subplot(2,1,2)
surf(squeeze(q0(:,:,2))./pi*180,squeeze(q2(:,:,2))./pi*180,squeeze(V_1(:,:,2)),squeeze(CV_1(:,:,2)),...
    'CDataMapping','direct');
xlabel('q_0 [deg]'); ylabel('q_2 [deg]'); zlabel('input V1 (-vel)');
col_leg = colorbar;
caxis([0 cm_length]);
...
figure; scr r;
subplot(2,1,1)
surf(squeeze(q0(:,:,1))./pi*180,squeeze(q2(:,:,1))./pi*180,squeeze(V_2(:,:,1)),squeeze(CV_2(:,:,1)),...
    'CDataMapping','direct');
xlabel('q_0 [deg]'); ylabel('q_2 [deg]'); zlabel('input V2 (+vel)');
subplot(2,1,2)
surf(squeeze(q0(:,:,2))./pi*180,squeeze(q2(:,:,2))./pi*180,squeeze(V_2(:,:,2)),squeeze(CV_2(:,:,2)),...
    'CDataMapping','direct');
xlabel('q_0 [deg]'); ylabel('q_2 [deg]'); zlabel('input V2 (-vel)');
col_trunk = colorbar;
caxis([0 cm_length]);
