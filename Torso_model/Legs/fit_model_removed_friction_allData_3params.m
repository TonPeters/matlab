...
for i=1:1:3
    tau_grav_tot(1+(i-1)*n_s:n_s*i) = tau_grav{i};
    q0_m_tot(1+(i-1)*n_s:n_s*i) = q0_m_s;
    q2_tot(1+(i-1)*n_s:n_s*i) = ones(n_s,1).*q2_list(i);
end
...
param0 =    [lcm1*m1+l1*m2+l1*m3;    lcm2*m2+l2*m3; m3_lcm3];
lb =        [0; 0; 0];
ub =        [100; 100; 100];
...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)costfunc_gravity_nonlin_3params_V1(param,q0_m_tot,tau_grav_tot,q2_tot),...
