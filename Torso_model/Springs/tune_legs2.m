...
q2_ = linspace(th_2_min,th_2_max,n_2);
...
tau_lim_trunk = [tau_nom_max,-tau_nom_max;tau_nom_max,-tau_nom_max].*(r_sp*r_gear2);
...
for j=1:1:n_2
    ax1(1) = subplot(2,1,1);
    plot(squeeze(q0(j,:,1)).'./pi*180,squeeze(tau_leg(j,:,1)).','color',ps.list_div_L{iter,5-j}); hold all; 
    ylabel('Force (+ vel) [N]');
    ax1(2) = subplot(2,1,2);
    plot(squeeze(q0(j,:,2)).'./pi*180,squeeze(tau_leg(j,:,2)).','color',ps.list_div_L{iter,5-j});hold all; 
    ylabel('Force (- vel) [N]');
...
load F_leg_zero.mat
axes(ax1(1))
plot(squeeze(q0_leg(1,:))./pi*180,squeeze(F_leg(1,:)),'linewidth',1,'color',ps.list_div_L{iter,5});
axes(ax1(2))
plot(squeeze(q0_leg(2,:))./pi*180,squeeze(F_leg(2,:)),'linewidth',1,'color',ps.list_div_L{iter,5});

% fig2 = figure; scr r;
% ax1(3) = subplot(2,1,1);
% plot(squeeze(q2(:,:,1))./pi*180,squeeze(tau_trunk(:,:,1)));hold all; 
% plot(th_2_lim,tau_lim_trunk,'r--'); title(['Trunk (shoulder load ',num2str(M_arms),' Nm)']);
% ylabel('Force (+ vel) [N]');
% plot(th_2_lim,tau_lim2_trunk,'k--');
% ax1(4) = subplot(2,1,2);
% plot(squeeze(q2(:,:,1))./pi*180,squeeze(tau_trunk(:,:,2)));hold all; 
% plot(th_2_lim,tau_lim_trunk,'r--');
% plot(th_2_lim,tau_lim2_trunk,'k--');
% ylabel('Force (- vel) [N]');
...
% 
% % plot zero
% load F_trunk_zero.mat
% axes(ax1(3))
% plot(squeeze(q2_trunk(1,:))./pi*180,squeeze(F_trunk(1,:)),'k','linewidth',2);
% axes(ax1(4))
% plot(squeeze(q2_trunk(2,:))./pi*180,squeeze(F_trunk(2,:)),'k','linewidth',2);
end

axes(ax1(1));
plot(th_0_lim,tau_lim_leg,'r--'); title('Legs');
plot(th_0_lim,tau_lim2_leg,'k--');
linkaxes(ax1(1:2),'x')
xlim(th_0_lim);
axes(ax1(2));
plot(th_0_lim,tau_lim_leg,'r--'); 
plot(th_0_lim,tau_lim2_leg,'k--');


leg_list2 = {'q_{2,min}, 0Nm','q_{2,max}, 0Nm','center, 0Nm','q_{2,min}, 52Nm','q_{2,max}, 52Nm','center, 52Nm'};
legend_outside(fig1,leg_list2)
...
filename = ['Legs_tuned_N',num2str(N_leg),'_F',num2str(F1_leg)];
% save_report(fig1,'/home/ton/Dropbox/Linux/Report/Images/Springs/',filename,'customleg',[8 8]);
% filename = ['Trunk_tuned_N',num2str(N_trunk),'_F',num2str(F1_trunk)];
