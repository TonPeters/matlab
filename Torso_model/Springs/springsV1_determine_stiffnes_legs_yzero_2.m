...
% 
% new_inertia = false;
inertia_name = 'legs_inertia_yzero';
% 
% fig1 = figure;
% for M_arms = [0 52]
%% springs
...
% q2_200 = shoulders_zero(th_0_max,0.200);
% q2_up = angle0_to_angle1(th_0_max)-th_0_max;
% q2_ = [th_2_min, q2_down, q2_200, th_2_max];
...
q2_leg = q2;
q0_leg = q0;
save('F_leg_zero','F_leg','q2_leg','q0_leg');

%% show results 2D
% % motor limits
% tau_lim = [tau_nom_max,-tau_nom_max;tau_nom_max,-tau_nom_max].*(r_sp*r_gear1);
...
% figure(fig1); scr r;
% ax1(1) = subplot(2,1,1);
% % plot(th_0_lim,tau_lim,'r--');
% plot(squeeze(q0(1,:)).'./pi*180,squeeze(tau_leg(1,:)).'); hold all; 
% ylabel('Force (+ vel) [N]');  title('Legs (shoulders centred)');
...
% % for i=2:length(q2_)-1
% %     leg_list{i+2} = '';
% % end
% % leg_list{length(q2_)+2} = 'q_{max}';
% legend_outside(fig1,{'load 0 Nm','load 52 Nm'});
% 
% 
% all_grids_on();
% set(ax1(1),'xticklabel',[]);
% % filename = ['Legs_Shoulders0_N',num2str(N_leg),'_F',num2str(F1_leg)];
