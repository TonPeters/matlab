...
F1_leg = 800;       % load of springs in the leg
Motor_limit = 1500;
...
for i=1:2
    for j=1:2
        ...
        for k=1:length(q2)
            %% Run simulation
            F_leg = sim_leg2(q0,q2(k),qd(i),qdd,m_4,M_4(j),N_leg,F1_leg);
        ...
        M_4c = 0;
        qdc = -1;
        F_legc = sim_leg2(q0c,q2c,qd(i),qdd,m_4,M_4(j),N_leg,F1_leg);
        ph_c((i-1)*2+j) = plot(q0c./pi*180,F_legc,'color',ps.tuegreen); hold all;
...
if fig_save
    ...
    dir_file = '/home/ton/Dropbox/Linux/Report_final/Images/Torso/leg_no_springs';
