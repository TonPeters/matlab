...
% joint angles
q0_ = linspace(th_0_min,th_0_max,2);
...
% load F_leg_zero.mat
% axes(ax1(1))
% plot(squeeze(q0_leg(1,:))./pi*180,squeeze(F_leg(1,:)),'k','linewidth',2);
% axes(ax1(2))
% plot(squeeze(q0_leg(2,:))./pi*180,squeeze(F_leg(2,:)),'k','linewidth',2);
...
linkaxes(ax1(3:4),'x')
xlim(th_2_lim);
plot(th_2_lim,tau_lim_trunk,'r--'); title('Trunk');
...
leg_list1 = {'q_{0,min}, 0Nm','q_{0,max}, 0Nm','center, 0Nm','q_{0,min}, 52Nm','q_{0,max}, 52Nm','center, 52Nm'};
legend_outside(fig2,leg_list1)


all_grids_on();
set([ax1(3)],'xticklabel',[]);
