...
% for j=1:1:n
%     ax1(1) = subplot(2,1,1);
%     plot(squeeze(q0(j,:,1)).'./pi*180,squeeze(tau_leg(j,:,1)).','color',ps.list_div_L{1,6-j}); hold all; 
%     ylabel('Force (+ vel) [N]');
%     ax1(2) = subplot(2,1,2);
%     plot(squeeze(q0(j,:,2)).'./pi*180,squeeze(tau_leg(j,:,2)).','color',ps.list_div_L{1,6-j});hold all; 
%     ylabel('Force (- vel) [N]');
...
xlabel('angle joint 1 [deg]');
axes(ax1(1));
plot(th_0_lim,tau_lim_leg,'r--'); title(['Legs (shoulder load ',num2str(M_arms),' Nm)']);
...
load F_leg_zero.mat
axes(ax1(1))
plot(squeeze(q0_leg(1,:))./pi*180,squeeze(F_leg(1,:)),'k','linewidth',1);
axes(ax1(2))
plot(squeeze(q0_leg(2,:))./pi*180,squeeze(F_leg(2,:)),'k','linewidth',1);
...
axes(ax1(3))
plot(squeeze(q2_trunk(1,:))./pi*180,squeeze(F_trunk(1,:)),'k','linewidth',1);
axes(ax1(4))
plot(squeeze(q2_trunk(2,:))./pi*180,squeeze(F_trunk(2,:)),'k','linewidth',1);
...
leg_list1 = {};
for i=1:n
    leg_list1{i} = num2str(q0_(i));
    leg_list2{i} = num2str(q2_(i));
end

legend_outside(fig1,leg_list2)
legend_outside(fig2,leg_list1)


all_grids_on();
set([ax1(1) ax1(3)],'xticklabel',[]);
filename = ['Legs_N',num2str(N_leg),'_F',num2str(F1_leg),'_Ma',num2str(M_arms),'_comb'];
save_report(fig1,'/home/ton/Dropbox/Linux/Report/Images/Springs/',filename,'customleg',[8 8]);
filename = ['Trunk_N',num2str(N_trunk),'_F',num2str(F1_trunk),'_Ma',num2str(M_arms),'_comb'];
