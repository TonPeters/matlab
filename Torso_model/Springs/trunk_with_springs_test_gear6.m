...
F1_trunk = 625;       % load of springs in the leg
...
% zero shoulders
q0c = linspace(th_0_min,th_0_max,n_2);
...
for ii = 1:2
    for i=1:2
        for j=1:2
            ...
            for k=1:length(q0)
                %% Run simulation
                F_trunk = sim_trunk2_test_gear6(q0(k),q2,qd(i),qdd,m_4,M_4(j),N_trunk(ii),F1_trunk);

                %% show results 2D
                % motor limits
                F_lim_trunk = [tau_nom_max,-tau_nom_max;tau_nom_max,-tau_nom_max].*(r_sp*r_gear2);
                F_lim2_trunk = [1500/r_sp/r_gear2,-1500/r_sp/r_gear2;1500/r_sp/r_gear2,-1500/r_sp/r_gear2].*(r_sp*r_gear2);
                % angle limits
                ...
                % plot max
                if ii==1
                    ph_q2(k,(i-1)*2+j) = plot(q2./pi*180,F_trunk,'color',ps.list_div_XL{end,end-5}); hold all;
                else
                    ph_q2(k,(i-1)*2+j) = plot(q2./pi*180,F_trunk,'color',ps.list_div_XL{1,end-k*3+2}); hold all;
                end
                
                leg_q0{k} = ['$q_0=',num2str(q0(k)),'$'];
            ...
            M_4c = 0;
            qdc = -1;
            F_trunkc = sim_trunk2_test_gear6(q0c,q2c,qd(i),qdd,m_4,M_4(j),N_trunk(ii),F1_trunk);
            if ii==1
                ph_c_g((i-1)*2+j) = plot(q2c./pi*180,F_trunkc,'color',ps.list_div_XL{end,end-5}); hold all;
            else
                ph_c((i-1)*2+j) = plot(q2c./pi*180,F_trunkc,'color',ps.list_div_XL{2,7}); hold all;
            end

            % motor limits
            ph_l((i-1)*2+j,:) = plot(th_2_lim,F_lim_trunk,'--','color',ps.list_div_XL{3,7}); 
        end
...
leg_list = {'$q_0$ min','$q_0$ max','center','motor limits','no springs'};
...
if fig_save
    ...
    dir_file = '/home/ton/Dropbox/Linux/Report_final/Images/Torso/trunk_with_springs_600';
