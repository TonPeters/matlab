...
for meas = measurements
    for set = sets
        ...
        %% optimization
        param0 =    [l_cm1*m_1+l_1*(m_2+m_3+m_4) ,l_cm2*m_2+l_2*(m_3+m_4), 10];
        lb =        [0,0,-1000];
        ub =        [10,10,1000];
        ...
        problem = createOptimProblem('lsqnonlin','objective',...
            @(param)cost_single_f23_coul(param,sdata{meas}.q0_s{set},sdata{meas}.tau_f{set},sdata{meas}.q2,sdata{meas}.qd0{set}),...
        ...
        % model calculation
        tau_est{meas,set} = -cost_single_f23_coul(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,sdata{meas}.qd0{set});
