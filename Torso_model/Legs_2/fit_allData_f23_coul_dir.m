...
for meas = measurements
    ...
    dir_est = 0.0096;
    coul_est = 0.0291;
    param0 =    [l_cm1*m_1+l_1*(m_2+m_3+m_4) ,l_cm2*m_2+l_2*(m_3+m_4),          coul_est,dir_est];
    lb =        [l_cm1*m_1min+l_1*(m_2min+m_3+m_4) ,l_cm2*m_2min+l_2*(m_3+m_4), 0,  0];
    ub =        [l_cm1*m_1max+l_1*(m_2max+m_3+m_4) ,l_cm2*m_2max+l_2*(m_3+m_4), 1,  1];
    ...
    problem = createOptimProblem('lsqnonlin','objective',...
        @(param)cost_single_f23_coul_dir(param,dat_q0,dat_tau,sdata{meas}.q2,dat_qd0),...
    ...
    % model calculation
    tau_est{meas,1} = -cost_single_f23_coul_dir(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est{meas,2} = -cost_single_f23_coul_dir(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);
