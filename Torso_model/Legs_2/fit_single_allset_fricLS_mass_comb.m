...
%% optimization
% param = m_1, m_2, Kls, Ksh, Kcoul
param0 =    [4.0,   4.8,    0.2120,    -0.0065, 0.0095];
lb =        [1.5,   1,    0,      -0.11,     0];
ub =        [9,     9,      0.5,     0.001,     0.3];
param0 =    [4.0,   4.8,    0.2120,    -0.000, 0.0095];
lb =        [0,   0,    0,      -0.0001,     0];
ub =        [9,     9,      0.5,     0.0001,     0.3];
...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)cost_legs_fricLS_mass_comb(param,dat_q0,dat_tau,dat_q2,dat_qd0),...
...
for meas = measurements
    ...
    % model calculation
    tau_est_p{meas,set} = -cost_legs_fricLS_mass_comb(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est_n{meas,set} = -cost_legs_fricLS_mass_comb(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);

    figure(fig1);
    plot(q0_est,tau_est_p{meas,set},'color',ps.list_div{meas,2});
    plot(q0_est,tau_est_n{meas,set},'color',ps.list_div{meas,2});
end
tau_mod = -cost_legs_fricLS_mass_comb([param(1),param(2),0,0,0],q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
