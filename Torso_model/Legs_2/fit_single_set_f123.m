...
%% optimization
param0 =    [10, l_cm1*m_1+l_1*(m_2+m_3+m_4) ,l_cm2*m_2+l_2*(m_3+m_4) ];
lb =        [-100000,0,0];
ub =        [1000000,10,10];
...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)cost_single_f123(param,sdata{meas}.q0_s{set},sdata{meas}.tau_f{set},sdata{meas}.q2),...
...
param
P1 = param(1);
P2 = param(2);                    % P1 = lcm1*m1 + l1*m2 + l1*m3 + l1*m4
P3 = param(3);                    % P2 = lcm2*m2 + l2*m3 + l2*m4
P4 = 2.4724;                      % P4 = lcm3*m3 +l3*m4


% model calculation
q0_est = linspace(th_0_min,th_0_max,40).';
tau_est = -cost_single_f123(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2);
