...
for meas = measurements
    ...
    for set = sets
        figure(fig1);
        ax = 2;
        if sdata{meas}.qd0{set}>0, ax = 1; end
        subplot(1,2,ax);        
%         plot(sdata{meas}.q0_m{set},sdata{meas}.tau_m{set},'color',ps.list_div{meas,1}); hold all;
        plot(sdata{meas}.q0_s{set},sdata{meas}.tau_f{set},'-','color',ps.list_div{meas,1}); hold all;
...
for meas = measurements
    ...
    % model calculation
    tau_est_p{meas,set} = -cost_comb_fricPC_massP2(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est_n{meas,set} = -cost_comb_fricPC_massP2(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);
