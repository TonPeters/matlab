...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)cost_comb_fricC_massP3(param,dat_q0,dat_tau,dat_q2,dat_qd0),...
...
for meas = measurements
    ...
    % model calculation
    tau_est_p{meas,set} = -cost_comb_fricC_massP3(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est_n{meas,set} = -cost_comb_fricC_massP3(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);
