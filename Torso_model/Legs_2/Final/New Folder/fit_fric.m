...
for meas = measurements
    ...
    tau_diff = [tau_diff;(((sdata{meas}.tau_f{1}+sdata{meas}.tau_f{2})./2-(sdata{meas}.tau_f{3}+sdata{meas}.tau_f{3})./2)./2)];
    tau_mean = [tau_mean ;((sdata{meas}.tau_f{1}+sdata{meas}.tau_f{2}+sdata{meas}.tau_f{3}+sdata{meas}.tau_f{3})./4)];
    
    figure(fig2); 
    plot(sdata{meas}.q0_s{1},(((sdata{meas}.tau_f{1}+sdata{meas}.tau_f{2})./2-(sdata{meas}.tau_f{3}+sdata{meas}.tau_f{3})./2)./2),'color',ps.list_div{meas,2}); hold all;
...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)cost_fricPC(param,tau_mean,tau_diff),...
...
if (exitflag ~= 1)
    ...
    %% show results
for meas = measurements
    tau_mean2 =((sdata{meas}.tau_f{1}+sdata{meas}.tau_f{2}+sdata{meas}.tau_f{3}+sdata{meas}.tau_f{3})./4);
    % model calculation
    tau_est_p = cost_fricPC(param,tau_mean2,zeros(size(tau_mean2)));

    figure(fig2);
    plot(sdata{meas}.q0_s{1},tau_est_p,'color',ps.list_div{meas,2});
    
    figure(fig1);
    for set = [1 3]
        plot(sdata{meas}.q0_s{set},(sdata{meas}.tau_f{set}-param(2)*sign(sdata{meas}.qd0{set}))*(1-param(1)*sign(sdata{meas}.qd0{set})),'color',ps.list_div{meas,2});
    end
end
...
meas = 1;
tau_mean2 =((sdata{meas}.tau_f{1}+sdata{meas}.tau_f{2}+sdata{meas}.tau_f{3}+sdata{meas}.tau_f{3})./4);
tau_est_p = cost_fricPC([0.9 0.00],tau_mean2,zeros(size(tau_mean2)));
