...
for meas = measurements
    ...
    dat_tau = (sdata{meas}.tau_f{1}+sdata{meas}.tau_f{2}+sdata{meas}.tau_f{3}+sdata{meas}.tau_f{4})./4;
    dat_q0 =sdata{meas}.q0_s{1};
    dat_q2 = sdata{meas}.q2;
    dat_qd0 = 1;
    
    figure(fig1);
    plot(sdata{meas}.q0_s{1},dat_tau,'color',ps.list_div{meas,1}); hold all;
    for set = sets
        figure(fig1);
...
%         dat_q2 = [dat_q2;ones(size(sdata{meas}.q0_s{set})).*sdata{meas}.q2];
    ...
    problem = createOptimProblem('lsqnonlin','objective',...
        @(param)cost_legs_mass(param,dat_q0,dat_tau,dat_q2,dat_qd0),...
    ...
    % model calculation
    tau_est_p{meas,set} = -cost_legs_mass(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est_n{meas,set} = -cost_legs_mass(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);
...
%% Test
meas = meas+1;
if meas>=length(ps.list_div), meas = 1; end;
tau_est = -cost_legs_mass([6.2 4.8],q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
plot(q0_est,tau_est,'color',ps.list_div{meas,2});
%% check params
for i=measurements
    par(i,:) = params{i,1};
end
