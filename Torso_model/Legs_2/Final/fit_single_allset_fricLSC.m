...
for meas = measurements
    ...
    problem = createOptimProblem('lsqnonlin','objective',...
        @(param)cost_legs_fricLSC(param,dat_q0,dat_tau,dat_q2,dat_qd0),...
    ...
    % model calculation
    tau_est_p{meas,set} = -cost_legs_fricLSC(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est_n{meas,set} = -cost_legs_fricLSC(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);
