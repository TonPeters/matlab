...
for meas = measurements
    ...
    problem = createOptimProblem('lsqnonlin','objective',...
        @(param)cost_legs_fricLSD_mass_P(param,dat_q0,dat_tau,dat_q2,dat_qd0),...
    ...
    % model calculation
    tau_est_p{meas,set} = -cost_legs_fricLSD_mass_P(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est_n{meas,set} = -cost_legs_fricLSD_mass_P(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);

    figure(fig1);
    pl_fitp(meas) = plot(q0_est,tau_est_p{meas,set},'color',ps.list_div{meas,2});
    pl_fitn(meas) = plot(q0_est,tau_est_n{meas,set},'color',ps.list_div{meas,2});
end
for meas = measurements
    uistack(pl_fitp(meas),'top');
    uistack(pl_fitn(meas),'top');
end
