...
for meas = measurements
    for set = sets
        ...
        %% optimization
        param0 =    [l_cm1*m_1+l_1*(m_2+m_3+m_4) ,l_cm2*m_2+l_2*(m_3+m_4),2.4724+l_3*m_4 ];
        lb =        [0,0,0];
        ub =        [10,10,10];
        ...
        problem = createOptimProblem('lsqnonlin','objective',...
            @(param)cost_single_f234(param,sdata{meas}.q0_s{set},sdata{meas}.tau_f{set},sdata{meas}.q2),...
        ...
        P4 = param(3);                      % P4 = lcm3*m3 +l3*m4
%         P4 = 2.4724;                      % P4 = lcm3*m3 +l3*m4


        % model calculation
        tau_est{meas,set} = -cost_single_f234(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2);
