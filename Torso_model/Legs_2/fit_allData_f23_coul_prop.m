...
param0 =    [l_cm1*m_1+l_1*(m_2+m_3+m_4) ,l_cm2*m_2+l_2*(m_3+m_4), 0.1,1.2,0];
lb =        [0,0,0,0,-100];
ub =        [10,10,1000,10,100];
...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)cost_single_f23_coul_prop(param,dat_q0,dat_tau,dat_q2,dat_qd0),...
...
P5 = param(3);                    % coul_fric;

for meas = measurements
    % model calculation
    tau_est{meas,1} = -cost_single_f23_coul_prop(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,0.004);
    tau_est{meas,2} = -cost_single_f23_coul_prop(param,q0_est,zeros(size(q0_est)),sdata{meas}.q2,-0.004);
