...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)cost_trunk_mass(param,dat_q2,dat_tau,dat_m2,dat_q0_offset,dat_qd2),...
...
%     for set = sets
        % model calculation
        tau_est_p{meas,set} = cost_trunk_mass(param,q2_est,zeros(size(q2_est)),sdata{meas}.m2,sdata{meas}.q_offset,1);
        tau_est_n{meas,set} = cost_trunk_mass(param,q2_est,zeros(size(q2_est)),sdata{meas}.m2,sdata{meas}.q_offset,-1);
