...
for meas = measurements
    ...
    % select gravity area
    qzero = (dat_q2-dat_q0_offset+2/180*pi);
%     figure
%     plot(dat_q2,cos(qzero)); hold all;
%     plot(dat_q2,(qzero));
%     plot(dat_q2,ones(size(dat_q2)).*pi/2);
    indi = find(qzero>pi/2-0.1 & qzero<pi/2+0.1);
    dat_tau= dat_tau(indi);
    dat_q2= dat_q2(indi);
    dat_qd2= dat_qd2(indi);
    
%     plot(dat_q2,qzero(indi))
%     pause
...
%     Ksp = param(4);
    ...
    problem = createOptimProblem('lsqnonlin','objective',...
        @(param)cost_trunk_fricCD(param,dat_q2,dat_tau,dat_m2,dat_q0_offset,dat_qd2),...
    ...
    % model calculation
    tau_est_p{meas,set} = cost_trunk_fricCD(param,q2_est,zeros(size(q2_est)),dat_m2,dat_q0_offset,1);
...
avg_par_min3 = mean(par([1 2 4 5],:))

figure;
subplot(2,1,1);
plot(par(1:3,1)); hold all;
plot(par(4:5,1));
ylabel('coul');
subplot(2,1,2);
plot(par(1:3,2)); hold all;
plot(par(4:5,2));
ylabel('dir');
