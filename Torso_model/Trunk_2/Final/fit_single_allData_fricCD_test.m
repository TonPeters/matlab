...
%     if meas==3, continue; end;
    ...
    for set = sets
        ...
        dat_q0_offset = [dat_q0_offset;sdata{meas}.q_offset.*ones(n,1)];
        dat_m2 = [dat_m2;sdata{meas}.m2.*ones(n,1)];
    end
end
set = 1;
...
problem = createOptimProblem('lsqnonlin','objective',...
    @(param)cost_trunk_fricCD_mass_test(param,dat_q2,dat_tau,dat_m2,dat_q0_offset,dat_qd2),...
...
%     for set = sets
        % model calculation
        tau_est_p{meas,set} = cost_trunk_fricCD_mass_test(param,q2_est,zeros(size(q2_est)),sdata{meas}.m2,sdata{meas}.q_offset,1);
        tau_est_n{meas,set} = cost_trunk_fricCD_mass_test(param,q2_est,zeros(size(q2_est)),sdata{meas}.m2,sdata{meas}.q_offset,-1);
