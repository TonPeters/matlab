...
%         plot(sdata{meas}.q2_s{set},sdata{meas}.tau_f{set},'--','color',ps.list_div{meas,2});
    ...
    dat_tau = (sdata{meas}.tau_f{1}+sdata{meas}.tau_f{2})./2;
    dat_q2 = sdata{meas}.q2_s{1};
    dat_qd2 = sdata{meas}.q2_s{1};
    plot(dat_q2,dat_tau,'color',ps.list_div{meas,1},'linewidth',3); hold all;
...
%     Ksp = param(4);
    ...
    % model calculation
    tau_est_p{meas,set} = cost_nonlin(param,q2_est,zeros(size(q2_est)),dat_m2,dat_q0_offset,1);
    tau_est_n{meas,set} = cost_nonlin(param,q2_est,zeros(size(q2_est)),dat_m2,dat_q0_offset,-1);
