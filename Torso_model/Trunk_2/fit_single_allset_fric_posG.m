...
for meas = measurements
    ...
    % cut gravitation point
    indi=  find((dat_q2-dat_q0_offset+2/180*pi)<pi/2);
...
%     Ksp = param(4);
    ...
    problem = createOptimProblem('lsqnonlin','objective',...
        @(param)cost_trunk_fric(param,dat_q2,dat_tau,dat_m2,dat_q0_offset,dat_qd2),...
    ...
    % model calculation
    tau_est_p{meas,set} = cost_trunk_fric(param,q2_est,zeros(size(q2_est)),dat_m2,dat_q0_offset,1);
    tau_est_n{meas,set} = cost_trunk_fric(param,q2_est,zeros(size(q2_est)),dat_m2,dat_q0_offset,-1);
