...
% data = importdata([filedir,filename]);
vectorsizes = [4, 4, 4];
...
% wheel2 = 2;
% u = trace{1}.signal{wheel2}(cut_start:end-shift);
% c = trace{2}.signal{wheel2}(cut_start:end-shift);
% e = trace{3}.signal{wheel2}(cut_start+shift:end);
...
ylabel('Sensitivity'); hold all;legend('S d-u','PS d-e');
subplot(4,1,4); 
