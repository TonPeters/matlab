...
%     plot(time,u(:,joint)); legend('PD','ffw','C')
    ...
    plot(time,cumsum(err(:,joint))./1000); ylabel('$e_y$ [m]'); hold all;
    ax(i_p) = subplot(n_plots,1,i_p);i_p = i_p+1;
    plot(time,err(:,joint)); ylabel('$\dot{e}_y$ [m/s]'); hold all;
...
set(ax(1),'ylim',[-0.035, 0.035]);
all_grids_on();
set(ax(1),'xticklabel',[]);

leg = legend_outside(fig1,{'No feed forward','$K_c$','$K_c$ and $K_a$','$\ddot{r}_y$ Reference'});
