...
%     plot(time,u(:,joint)); legend('PD','ffw','C')
    ...
    plot(time,cumsum(err(:,joint))./1000); ylabel('$e_x$ [m]'); hold all;
    ax(i_p) = subplot(n_plots,1,i_p);i_p = i_p+1;
    plot(time,err(:,joint)); ylabel('$\dot{e}_x$ [m/s]'); hold all;
...
plot(time,acc*0.015,'k');
axes(ax(2));
plot(time,acc.*0.13,'k');

xlabel('Time [s]');
linkaxes(ax,'x');
set(ax(2),'xlim',[0 23]);
set(ax(2),'ylim',[-0.15, 0.15]);
set(ax(1),'ylim',[-0.016, 0.016]);
all_grids_on();
set(ax(1),'xticklabel',[]);

leg = legend_outside(fig1,{'No feed forward','$K_c$','$K_c$ and $K_a$','$\ddot{r}_x$ Reference'});
